...
// -----------------------------------------------------------------------------
    ...
    void met4(){
        ...
        Klientas[] a=new Klientas[n];
        Klientas[] result =new Klientas[n];

        for(int i=0;i<n;i++) {
            a[i] = new Klientas("SWED", 12 + i, 174 * i * 0.98);
        }
        ...
        int maxAmzius = 50;
        int minAmzius = 18;
        double minIndelis = 1000;

        System.out.println("Atrenkame tinkamus paskolai vartotojus");

        for(int i=0;i<n;i++) {
            if(a[i].getAmžius() >= minAmzius && a[i].getAmžius() <= maxAmzius && a[i].getIndėlis()>=minIndelis)
                System.out.println(a[i]);
        }
