...
import darbas.GreitaveikosTyrimas;
...
public class Lab2Window extends JFrame implements ActionListener {
    ...
    private SortedSetADTx<Book> bookSet;
    ...
    private void customFunction()
    {
        ...
        Book from = BookMaker.getBook(2);
        Book to =  BookMaker.getBook(9);
        
        KsSwing.oun(taOutput, from);
        KsSwing.oun(taOutput, to);

        KsSwing.oun(taOutput, bookSet.subSet(from, true, to, true).toVisualizedString(delimiter));
        KsSwing.oun(taOutput, bookSet.subSet(from, true, to, false).toVisualizedString(delimiter));
        KsSwing.oun(taOutput, bookSet.subSet(from, false, to, true).toVisualizedString(delimiter));
        KsSwing.oun(taOutput, bookSet.subSet(from, false, to, false).toVisualizedString(delimiter));
        
        KsSwing.oun(taOutput, bookSet.subSet(from, to).toVisualizedString(delimiter));
    ...
    private void getHigher()
    {
        KsSwing.setFormatStartOfLine(true);
        Book from = BookMaker.getBook(2);
        KsSwing.oun(taOutput, bookSet.headSet(from));
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            if (source.equals(panButtons.getButtons().get(0))) {
                ...
                KsSwing.ounerr(taOutput, "Gauti poaaibi");
                customFunction();
            }else if(source.equals(panButtons.getButtons().get(6))) {
                KsSwing.setFormatStartOfLine(true);
                KsSwing.ounerr(taOutput, "Gauti aukstesni");
                getHigher();
                KsSwing.setFormatStartOfLine(false);
    ...
    public void treeGeneration(String filePath) throws MyException {
        ...
        Book[] bookArray = BookMaker.generate(10) ;
        ...
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, bookSet.toVisualizedString(delimiter),
    ...
    private void treeAdd() throws MyException {
        Book book = BookMaker.createBook();
        bookSet.add(book);
        panParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsSwing.setFormatStartOfLine(true);
        KsSwing.oun(taOutput, book, MESSAGES.getString("msg7"));
        KsSwing.oun(taOutput, bookSet.toVisualizedString(delimiter));
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (bookSet.isEmpty()) {
            ...
            Book auto = (Book) bookSet.toArray()[nr];
            bookSet.remove(auto);
    ...
    private void treeIteration() {
        ...
        if (bookSet.isEmpty()) {
            ...
            KsSwing.oun(taOutput, bookSet, MESSAGES.getString("msg8"));
        }
        KsSwing.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectedIndex()) {
            case 0:
                bookSet = new BstSetKTUx(new Book());
                break;
            case 1:
                bookSet = new AvlSetKTUx(new Book());
                break;
            default:
                enableButtons(false);
