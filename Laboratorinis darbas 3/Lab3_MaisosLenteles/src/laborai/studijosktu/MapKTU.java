 ...
 * pvz. klasės Book objektas, klasėje būtina perdengti metodus
...
public class MapKTU<K, V> implements MapADTp<K, V> {
    ...
    protected boolean numerationType = false;
    ...
    public List<K> keySetList = new ArrayList();
    
    public Node prevNode = new Node();
    
    public Node firstNode = new Node();
    ...
    //keyset suformuotu put metu 
    public List<K> keySet()
    {   
        for (Node<K, V> n = firstNode; n != null; n = n.linkedNext) 
        {
    ...
    {
        return (double)chainsCounter / (double)table.length;
    //  return (double)(table.length-emptyElementCount()) / (double)chainsCounter; 
    ...
    public V put(K key, V value) {
        ...
        if (node == null) {
            ...
            if(prevNode!=null)
            {
                prevNode.linkedNext = table[index];
            }
    ...
    private void rehash(Node<K, V> node) {
        MapKTU mapKTU = new MapKTU(table.length * 2, loadFactor, ht);
        for (int i = 0; i < table.length; i++) {
            while (table[i] != null) {
