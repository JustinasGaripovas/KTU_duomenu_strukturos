...
import laborai.demo.Book;
import laborai.demo.BookMaker;
...
public class Lab3Window extends JFrame implements ActionListener {
    ...
    private MapADTx<String, Book> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private final BookMaker makingBooks = new BookMaker();
    ...
    public void mapGeneration(String filePath) {
        ...
        createMap();
        // Jei failas nenurodytas - generuojami autoai ir talpinami atvaizdyje
        if (filePath == null) {
            Book[] bookArray = makingBooks.generate(sizeOfGenSet, sizeOfInitialSubSet);
            System.out.println("___________________________________________");
            for (Book a : bookArray) {
                String ide = makingBooks.gautiIsBazesbookId();
                Ks.oun(a);
                Ks.oun(ide);
                map.put(
                        ide,//raktas
    ...
    public void mapAdd() throws MyException {
        Book a = makingBooks.parduotibook();
        map.put(
                makingBooks.gautiIsBazesbookId(),// Raktas
                a);
        table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
                panParam1.getTfOfTable().get(5).getText(), map.getMaxChainSize()), colWidth);
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Book(), initialCapacity, loadFactor, ht);
                break;
            case 1:
                //map = new MapKTUOA<>(initialCapacity, loadFactor, ht);
                map = new MapKTUx<>(new String(), new Book(), initialCapacity, loadFactor, ht);
                break;
            default:
                IntStream.of(1, 3).forEach(p -> panButtons.getButtons().get(p).setEnabled(false));
